...
HIGH_RISK_FILES=$(git diff --name-only HEAD~1..HEAD 2>/dev/null | grep -E "auth|payment|billing|stripe|subscription|prisma/schema|middleware|webhook" || true)
...
if [[ $RISK_SCORE -ge 7 ]]; then
  ...
  # Runs: npm run test:medium 2>/dev/null || npm test
  npm run test:medium 2>/dev/null || npm test
